...
# All 3E variants inherit the winning architecture up to +30%:
# structural stop capped at 10%, +5% -> BE, +30% -> +10% floor.
# 365d is an observation ceiling, not a proposed live time-exit rule.
VARIANTS = {
    "AUDIT_180D_T30_L10": {"horizon": H180, "matched": H180},
    "OBS365_BASE_T30_L10": {"horizon": H365, "matched": H365},

    # Sparse milestone floors, then hard take-profit at +1000%.
    "MILESTONES_A_TP1000": {
        "horizon": H365, "matched": H365,
    ...
    # Hard profit-taking controls.
    "TP500_FULL": {"horizon": H365, "matched": H365, "take_profit": 5.00},
    "TP1000_FULL": {"horizon": H365, "matched": H365, "take_profit": 10.00},

    # Very loose late-stage peak trails, only after +200%.
    "P200_TRAIL40": {"horizon": H365, "matched": H365, "trail_trigger": 2.00, "trail_pct": 0.40},
    "P200_TRAIL50": {"horizon": H365, "matched": H365, "trail_trigger": 2.00, "trail_pct": 0.50},
    "P200_TRAIL60": {"horizon": H365, "matched": H365, "trail_trigger": 2.00, "trail_pct": 0.60},

    # Hybrid: protect +300% after a +500% move, then only tighten further if price keeps multiplying.
    "P500_L300_THEN_TRAIL50_TP1000": {
        "horizon": H365, "matched": H365,
        "floors": [(5.00, 3.00)],
        "trail_trigger": 5.00, "trail_pct": 0.50,
        "take_profit": 10.00,
    },
}
...
def full_window_available(df, sig, hours):
    return int(sig["entry_index"]) + int(hours) < len(df)


def finish(sig, variant, exit_time, reason, pnl, initial_stop, stop, activated,
           peak, trough, exit_ret, bars_held, floor_level, trail_active, censored):
    ...
    return {
        ...
        "trail_active": trail_active, "censored": censored,
...
def simulate(df, sig, variant, spec, cfg):
    ...
    for j in range(start, end):
        ...
        if not activated:
            ...
            # Existing stop always acts before a new milestone reached inside this hourly bar.
            if low <= stop:
            ...
            # Inherited Round 3C winner floor.
    ...
    # Observation-window end: mark censored rather than pretending this is the desired live exit.
    last = df.iloc[end - 1]
    close = float(last.close)
    pnl = net_pnl(entry, [(1.0, close)], cfg)
    return finish(sig, variant, last.time, "OBSERVATION_END", pnl, initial_stop, stop,
...
def process_symbol(symbol, btc, cfg):
    ...
    for sig in sigs:
        for variant, spec in VARIANTS.items():
            matched = int(spec.get("matched", spec["horizon"]))
            if not full_window_available(df, sig, matched):
                continue
...
def summarise(trades):
    ...
    for variant, g in q.groupby("variant"):
        ...
        w, l = p[p > 0], p[p < 0]
        rows.append({
            "variant": variant, "trades": len(g),
            "net_pnl_usdt": p.sum(), "expectancy_usdt": p.mean(),
        ...
        for y, gy in g.groupby("year"):
            ...
            years.append({
                ...
                "censored_rate": gy.censored.mean(),
            })
...
def main():
    ...
    out = Path("research/results/round3e"); out.mkdir(parents=True, exist_ok=True)
    ...
    if trades.empty:
        raise RuntimeError("Round 3E produced no trades")

    summary, yearly = summarise(trades)
    summary.to_csv(out / "summary.csv", index=False)
    yearly.to_csv(out / "year_summary.csv", index=False)

    # Threshold diagnostics on the 365d base cohort, before exits distort path availability.
    base = trades[trades.variant == "OBS365_BASE_T30_L10"].copy()
    thresh_rows = []
    for t in THRESHOLDS:
        hits = base.mfe_pct >= t
    ...
    manifest = {
        "study": "Round 3E large-winner exit architecture",
        "frozen_entry": "CONTROLLED_ACTIVITY from Round 2B",
        "inherited_base": "$300; structural stop capped at 10%; +5% -> breakeven; +30% -> +10% floor",
        "observation_window": "365 days; observation end is reported as censored and is not proposed as a live time-exit rule",
        "variants": VARIANTS,
        ...
        "purpose": "test sparse large-winner profit floors, hard +500%/+1000% profit-taking, and very loose late-stage peak trails without retuning entries",
