...
def feature_frame(df: pd.DataFrame, btc: pd.DataFrame) -> pd.DataFrame:
    ...
    for h in (24, 72, 120):
        hi = x.high.shift(1).rolling(h).max(); lo = x.low.shift(1).rolling(h).min()
        x[f"range_{h}h"] = hi / lo - 1
    ...
    x["taker_6h"] = taker.shift(1).rolling(6).mean()
    x["taker_24h"] = taker.shift(1).rolling(24).mean()
    ...
    lows=x.low.shift(1)
    x["higher_lows_24h"]=(lows>lows.shift(6)).rolling(24).mean()
    x["green_share_24h"]=(x.close>x.open).shift(1).rolling(24).mean()
    x["btc_ret_24h"]=b.close.pct_change(24).reindex(x.index)
    x["btc_ret_72h"]=b.close.pct_change(72).reindex(x.index)
...
def discover_events(symbol: str, df: pd.DataFrame, ff: pd.DataFrame) -> pd.DataFrame:
    # Hindsight labels only. Features are measured at candidate start, before the future window.
    ...
    upside = future_high/df.close-1
    adverse = future_low/df.close-1
    eligible=(ff.time>=pd.Timestamp("2019-01-08",tz="UTC")) & ff[FEATURES].notna().all(axis=1)
    # Strong rallies: >=20% within 7d and no worse than -8% in first 3d.
    winners=eligible & (upside>=0.20) & (adverse>=-0.08)
    # Controls: <=8% upside in 7d, matched from otherwise valid observations.
    controls=eligible & (upside<=0.08)
    # De-duplicate starts so one rally isn't counted dozens of times.
    keep_w=[]; last=-9999
    for i in np.flatnonzero(winners.to_numpy()):
        if i-last>=120: keep_w.append(i); last=i
    # Match each winner to nearest control in same symbol and +/-45 days, else closest prior control.
    ...
    for i in keep_w:
        ...
        if len(ctrl_idx):
            dt=np.abs((ff.time.iloc[ctrl_idx]-ff.time.iloc[i]).dt.total_seconds().to_numpy())
            mask=(dt<=45*86400)
...
def main():
    ...
    out=Path("research/results/round2a"); out.mkdir(parents=True,exist_ok=True)
    ...
    for symbol in cfg["symbols"]:
        ...
        ev=discover_events(symbol,df,ff)
        if not ev.empty: all_events.append(ev)
    events=pd.concat(all_events,ignore_index=True) if all_events else pd.DataFrame()
    events.to_csv(out/"rally_control_events.csv",index=False)
    ...
    print("events",len(events),"rallies",int((events.kind=="RALLY").sum()) if len(events) else 0)
